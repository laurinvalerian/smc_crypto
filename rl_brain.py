...
class CentralRLBrain:
    ...
    1. Call ``should_trade(obs, coin_id)`` before each potential entry.
       Returns ``(take_trade: bool, trade_id: str)``.

    2. Store the ``trade_id`` alongside the trade.

    3. Call ``record_outcome(trade_id, reward)`` when the trade (or skip)
       resolves. *reward* = PnL change in % (positive or negative).

    4. The brain auto-updates its policy every BUFFER_CAPACITY completed
       transitions and saves the model atomically to disk.
    """
    ...
    def __init__(
        ...
        # ── Trade-ID counter (monotonically increasing) ───────────
        self._trade_counter: int = 0

        # ── Pending trades: awaiting reward (keyed by trade_id) ───
        # Each value: {"obs": np.ndarray, "act": int, "logp": float, "val": float}
        self._pending_trades: dict[str, dict[str, Any]] = {}

        # ── Completed rollout buffer (obs/act/logp/val/rew/done aligned) ─
    ...
    def should_trade(self, obs: np.ndarray, coin_id: Any) -> tuple[bool, str]:
        """
        ...
        -------
        take_trade : bool
            True = take the trade, False = skip.
        trade_id : str
            Unique identifier for this decision. **Must** be passed back
            to ``record_outcome`` when the trade resolves.
        """
        # Generate a unique trade_id regardless of enabled state
        self._trade_counter += 1
        trade_id = f"t{self._trade_counter}"

        if not self._enabled:
            return True, trade_id  # pass-through if no PyTorch
        ...
        # Store in pending dict – will be moved to training buffer on
        # record_outcome(), guaranteeing correct obs<->reward mapping.
        self._pending_trades[trade_id] = {
            "obs": obs_with_coin.copy(),
            "act": act,
            "logp": float(log_prob.item()),
            "val": float(value.item()),
        }

        return act == 1, trade_id  # 1 = trade, 0 = skip
    ...
    def record_outcome(
        ...
        """
        Record the reward for a previous ``should_trade`` decision.

        Parameters
        ----------
        trade_id : str
            The identifier returned by ``should_trade``.
        reward : float
            Pure PnL change in % (e.g. +1.5 or -0.8).
        done : bool
            Marks end of episode (True for each closed trade or skip).
        """
        if not self._enabled:
            return

        pending = self._pending_trades.pop(trade_id, None)
        if pending is None:
            logger.warning(
                "CentralRLBrain: record_outcome called with unknown trade_id=%s "
                "(already resolved or never created). Ignoring.",
                trade_id,
            )
            return

        # Move the fully-paired transition into the training buffer
        self._obs_buf.append(pending["obs"])
        self._act_buf.append(pending["act"])
        self._logp_buf.append(pending["logp"])
        self._val_buf.append(pending["val"])
    ...
    def _ppo_update(self) -> None:
        ...
        logger.info(
            "CentralRLBrain: PPO update #%d completed (buffer=%d, pending=%d)",
            self._total_updates, n, len(self._pending_trades),
        )

        # Clear training buffer (pending_trades stays – they're still open)
        self._obs_buf.clear()
        self._act_buf.clear()
        ...
        # Atomic save after every update
        self._save_model()

    # ── Persistence ───────────────────────────────────────────────

    def _save_model(self) -> None:
        """Atomic save: write to .tmp first, then rename to prevent corruption."""
        if not self._enabled:
            return
        tmp_path = self._model_path.with_suffix(".pt.tmp")
        try:
            torch.save(
                {
            ...
            tmp_path.replace(self._model_path)  # atomic on POSIX
            logger.info(
                "CentralRLBrain: model saved (%d updates, %d coins) -> %s",
                self._total_updates, len(self._coin_to_idx), self._model_path,
            )
        except Exception as exc:
            logger.error("CentralRLBrain: model save FAILED: %s", exc)
    ...
    def _load_model(self) -> None:
        ...
        if not self._model_path.exists():
            logger.info("CentralRLBrain: no checkpoint found at %s – starting fresh.", self._model_path)
            return
        try:
            ...
            self._trade_counter = ckpt.get("trade_counter", 0)
            ...
            logger.info(
                "CentralRLBrain: ✓ model loaded successfully (%d prior updates, %d coins, counter=%d) from %s",
                self._total_updates, len(self._coin_to_idx), self._trade_counter, self._model_path,
            )
        except Exception as exc:
            logger.error(
                "CentralRLBrain: model load FAILED – starting fresh. Error: %s", exc,
            )
    ...
    def flush(self) -> None:
        ...
        if not self._enabled:
            return
        # Warn about pending (unresolved) trades that will be lost
        if self._pending_trades:
            logger.warning(
                "CentralRLBrain: flush() called with %d pending trades still "
                "awaiting outcomes – these transitions are discarded.",
