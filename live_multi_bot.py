        ...
        if len(self.buffer_15m) >= swing_len * 2:
            try:
                ...
                if zone_for_quality is None:
                    ...
                    # TEMPORARY DEBUG: log zone search results (only for first 3 symbols to limit spam)
                    if symbol in ("BTC/USDT:USDT", "EUR_USD", "AAPL"):
                        ...
                        fvg_cols = list(fvg_d.columns) if fvg_d is not None and not fvg_d.empty else []
                        ob_cols = list(ob_d.columns) if ob_d is not None and not ob_d.empty else []
                        self.logger.info(
                            "ZONE_PROBE %s | bias=%s fvgs=%d obs=%d zone_found=%s buf15m=%d fvg_cols=%s ob_cols=%s fvg_null=%s ob_null=%s",
                            symbol, daily_bias, n_fvg, n_ob,
                            zone_for_quality is not None, len(self.buffer_15m),
                            fvg_cols, ob_cols,
                            fvg_d is None, ob_d is None,
                        )
